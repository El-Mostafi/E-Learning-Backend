...
router.get("/api/courses/:id/sections", async (req: Request, res: Response, next: NextFunction) => {
  const courseId = req.params.id;
  const sections = await sectionService.findAll(courseId);
  res.send(sections);
});

router.get("/api/courses/:id/sections/:sectionId", async (req: Request, res: Response, next: NextFunction) => {
  const courseId = req.params.id;
...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    const result = await sectionService.create(courseId, sectionDto);
    res.send(result);
  }
);
...
router.put(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    const sectionId = req.params.sectionId;
    const sectionDto = req.body;
    const result = await sectionService.update(courseId, sectionId, sectionDto);
    res.send(result);
  }
);
...
router.delete(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    const courseId = req.params.id;
    const sectionId = req.params.sectionId;
    const result = await sectionService.delete(courseId, sectionId);
    res.send(result);
  }
);
